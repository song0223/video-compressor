...
压缩工具箱是一个基于 Tauri、React 和 Rust 的跨平台桌面小工具集合。当前版本专注于媒体压缩，支持视频压缩/转码和图片压缩，并内置 FFmpeg/FFprobe，普通用户下载安装包后不需要再额外安装命令行工具。
...
[压缩工具箱 0.3.0](https://github.com/song0223/video-compressor/releases/tag/v0.3.0)

- macOS Apple Silicon：`compression-toolbox_0.3.0_aarch64.dmg`
- Windows x64：`compression-toolbox_0.3.0_x64-setup.exe`
...
- 支持从列表中删除单个任务、清空列表、全部导出。

### 图片压缩

- 支持拖拽导入图片，也支持手动多选图片。
- 视频和图片功能使用独立的数据结构与接口，后续扩展时互不影响。
- 支持原格式输出，也支持压缩到 JPEG/WebP。
- 支持 `80%`、`60%`、`40%`、`10%` 默认压缩档位。
- 支持 `1%` 到 `99%` 自定义压缩百分比。
- 显示原始大小、预计大小和导出后的实际大小。
- 极限压缩会自动使用更适合小体积的输出策略。
